...
public class OrderServiceImpl implements OrderService {
	...
	public void saveOrder(Order order) {
		...
		if (orderItem != null) {
			...
			if (!StringUtils.isEmpty(order.getOrderItem().getOptionValueKeyGroup())) {
				...
				orderItem.setPrice(productList.get(0).getShopPrice());
			}
	...
	public void savePaymentHistory(Map<String,Object> mapReq) {
		...
		// 获取数据库联系人地址
		Contect contect = contectMapper.selectByPrimaryKey(Integer.parseInt(mapReq.get("contectId").toString()));
		OrderHistory orderHistory = new OrderHistory();
	...
	public Order getOrderItem(Order order) {
		...
		if (!StringUtils.isEmpty(order.getUserId())) {
			...
			if (map != null && map.get("contectList") != null) {
				List<Contect> contectList = (List<Contect>) map.get("contectList");
