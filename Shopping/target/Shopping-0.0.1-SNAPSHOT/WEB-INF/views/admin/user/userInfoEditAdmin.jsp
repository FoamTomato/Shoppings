								...
								<div class="hpanel">
									...
									<form id="form_userInfo" name="form_userInfo" action='<c:url value="/admin/user/save?${_csrf.parameterName}=${_csrf.token}" />' method="POST">
										...
										<div class="form-group col-lg-12">
											<label>用户角色</label>
											<div>
											<input type="checkbox" id="permisskey1" name="role" style="display:none" value="ROLE_ADMIN" checked>
												<c:forEach items="${role}" var="datalist" varStatus="status">
													<div class="form-group radio radio-success radio-inline" style="margin-top:3px; margin-bottom: 10px">
														<input type="radio" id="permisskey1" name="role" value="${datalist.roleName}"<c:if test="${user.jurisdiction==datalist.roleName}">checked</c:if> <c:if test="${status.first}">checked</c:if>>
														<label>${datalist.roleName}</label>
													</div>
												</c:forEach>
											</div>
										</div>
