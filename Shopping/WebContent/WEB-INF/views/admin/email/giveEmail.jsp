...
<div id="wrapper">
										    ...
										    <el-aside :width="productlist!=null?'700px':'100%'">
										      <el-container style="height:620px">
											    <el-header style="float:left;height: auto;font-family: unset">
											    	...
											    	<h5>时间：{{timechange(detailsList.emailTimes)}}</h5><a v-if="detailsList.spare3 != null" style="color:green;margin:5px" @click="downlib()">附件下载</a><a style="color:#6699CC;margin:5px" @click="dialogFormVisible = true">回复</a>
											    </el-header>
	...
	<!-- 接收邮件模板 -->
	<!-- Table -->
	<el-dialog title="" width="40%"  :before-close="handleClose" :visible.sync="dialogTableVisible">
...
<!-- 回复邮件 -->
<el-dialog title="邮件回复" :visible.sync="dialogFormVisible">
    <el-row :gutter="20">
	  <el-col :span="12">标题
		  <el-input
		    placeholder="请输入回复标题" v-model="detailsList.spare1"
		    >
		  </el-input>
	  </el-col>
	  <el-col :span="12">抄送
		  <el-input
		    placeholder="请输入抄送邮箱" v-model="csEmail"
		    >
		  </el-input>
	  </el-col>
	</el-row>
	<el-row :gutter="20">
	  <el-col :span="12">发件人<br/>
		  <el-select v-model="detailsList.emailGetman" placeholder="请选择发件人" style="width:100%">
		    <el-option
		      v-for="item in ooption"
		      :key="item.id"
	  ...
	  <el-col :span="12">收件人
		  <el-input
		    placeholder="请输入收件人" v-model="detailsList.emailPostman"
		    >
		  </el-input>
	  </el-col>
	</el-row>
	<el-row :gutter="20">
	  <el-col :span="24">
		<el-upload
		  ...
		  action="${pageContext.request.contextPath}/email/CommonsMultipartFile?${_csrf.parameterName}=${_csrf.token}"
		  :file-list="fileList"
		  :auto-upload="true">
		  <el-button slot="trigger" size="small" type="primary" style="margin:10px 0px">选取附件</el-button>
		</el-upload>
	  </el-col>
	</el-row>
	<el-row :gutter="20">
	  <el-col :span="24" >
		<el-input type="textarea" :autosize="{ minRows: 15, maxRows: 15}" placeholder="请输入内容" v-model="detailsList.text"></el-input>
	  </el-col>
	</el-row>
	<el-row :gutter="20">
	  <el-col :span="24" >
	  	<el-button type="primary" style="float:right;margin:10px 0px" @click="posts">发送</el-button>
	  </el-col>
	</el-row>
</el-dialog>
</div>
		...
		var vm=new Vue({
			...
			methods:{
		        ...
		        gets(){
					...
					let datas={}
	        	  	let list=""
					let i=this.multipleSelection
					for(let z=0;z<i.length;z++){
		        	  	list+=i[z].id
					}
					datas["value7"]=this.value7
					datas["list"]=list
					//ajiox请求方式
					this.$http.post("email/get/Email?${_csrf.parameterName}=${_csrf.token}",JSON.stringify(datas),{emulateJSON:true}).then(result=>{
						//判断是否添加成功
						if(result.body>0){
        	    ...
        	    downlib(){
        	    	//文件下载
        	    	let list=this.detailsList.spare3.split(",")
        	    	for(let i=0;i<list.length;i++){
        	    		  //必须同源才能下载
        	    	      let alink = document.createElement("a")
        	    	      alink.href = "http://localhost:8080/Shopping/resources/upload/"+list[i]
        	    	      alink.download =list[i] //图片名
        	    	      alink.click()
        	    	}
        	    },
        	    posts(){
        	    	//发送邮件
            	    /* this.data2.title=this.detailsList.spare1
            	    this.data2.csEmail=this.csEmail
            	    this.data2.postMan=this.detailsList.emailGetman
            	    this.data2.getMan=this.detailsList.emailPostman
            	    this.data2.content=this.detailsList.text
            	    this.data2.fjEmail=this.fileRepose.substr(0,this.fileRepose.length-1) */
        	    	let datas={}
					datas["title"]=this.detailsList.spare1
					datas["csEmail"]=this.csEmail
					datas["postMan"]=this.detailsList.emailGetman
					datas["getMan"]=this.detailsList.emailPostman
					datas["content"]=this.detailsList.text
					datas["fjEmail"]=this.fileRepose.substr(0,this.fileRepose.length-1)
					datas["fjEmail2"]=this.fileRepose2.substr(0,this.fileRepose2.length-1)
					console.log(this.fileRepose)
        	    	this.fileRepose=""
					//ajiox请求方式
					this.$http.post("email/reportEmail?${_csrf.parameterName}=${_csrf.token}",JSON.stringify(datas),{emulateJSON:true}).then(result=>{
						if(result.body=="1"){
							this.$message.success("成功发送"+result.body+"封邮件")
			        		this.dialogFormVisible= false//回复
						}else if(result.body=="0"){
        	    ...
        	    filesuccess(response, file, fileList){
        	    	//附件上传成功
        	    	this.fileRepose+=response.fileName+","
        	    	this.fileRepose2+=response.fileName.split("/resources/upload/")[1]+","
        	    }
			},
		    mounted() {
		    	// 实例化editor编辑器
