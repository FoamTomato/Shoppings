...
import com.sybinal.shop.service.defaultService;
import com.sybinal.shop.service.logOutOfService;
import com.sybinal.shop.service.eub.EUBService;
import com.sybinal.shop.service.hm.HMService;
...
import com.sybinal.shop.service.sf.SfService;
...
public class FLogisticsController {
	...
	@Autowired
	EUBService eubServices;

	@Autowired
	JpService jpServices;

	@Autowired
	SfService sfServices;
	...
	// public static String urls= "http://api.dyexpress.com";
	 ...
	 * 右侧界面状态保存orderSave
	 * 
	 * 项目名称：Shopping 类描述： 创建人：PC1 创建时间：2020年3月13日 下午2:27:59
	 * 
	 * @version
	 */
	...
	public int addHJLogistics(@RequestBody hjBase hjBases) {
		...
		data.put("fids", Arrays.asList(hjBases.getHjShipperhawbcode()));
		defaults.BatchDefaultAllOrders(data);

		logisticsService.addWeight(hjBases.getHjInvoiceweight(), hjBases.getHjShipperhawbcode(),
				hjBases.getHjReferenceno(), userService.Justiactions(username()).getStandby1());
		...
		hjBases.setHjReferenceno("HJ" + SnowIdUtils.uniqueLong());
	 ...
	 * 
	 * 项目名称：Shopping 类描述： 创建人：PC1 创建时间：2020年3月16日 上午10:46:36
	 * 
	 * @version
	 */
	...
	public int defaultHj(@RequestBody hjBase hjBases) {
		...
		if (hjBases.getHjStandy3().equals("0")) {
			...
			// 更新跟踪号
			logisticsService.defaulData(hjBases.getHjStandy4(), username(), hjBases.getHjShipperhawbcode(),
					hjBases.getHjShippingmethod(), hjBases.getHjConsignee());
			return i;
			...
			logisticsService.defaulData(hjBases.getHjStandy4(), username(), hjBases.getHjShipperhawbcode(),
					hjBases.getHjShippingmethod(), hjBases.getHjConsignee());
			return i;
		} else if (hjBases.getHjStandy3().equals("4")) {
			/**
			 * 顺丰
			 */
			int i = sfServices.default1(hjBases.getHjStandy2());
			// 更新跟踪号
			logisticsService.defaulData(hjBases.getHjStandy4(), username(), hjBases.getHjShipperhawbcode(),
					hjBases.getHjShippingmethod(), hjBases.getHjConsignee());
			return i;
		} else if (hjBases.getHjStandy3().equals("6")) {
			/**
			 * EUB
			 */
			int i = eubServices.default1(hjBases.getHjStandy2());
			// 更新跟踪号
			logisticsService.defaulData(hjBases.getHjStandy4(), username(), hjBases.getHjShipperhawbcode(),
					hjBases.getHjShippingmethod(), hjBases.getHjConsignee());
			return i;
		} else if (hjBases.getHjStandy3().equals("7")) {
			/**
			 * 黑猫
			 */
			int i = hmService.default1(hjBases.getHjStandy2());
			// 更新跟踪号
			logisticsService.defaulData(hjBases.getHjStandy4(), username(), hjBases.getHjShipperhawbcode(),
					hjBases.getHjShippingmethod(), hjBases.getHjConsignee());
			return i;
	...
	public int hj_logistics_post(@RequestBody hjBase hjBases) {
		...
		return logisticsService.postLogistics(hjBases.getHjStandy8(),
				userService.Justiactions(username()).getStandby1());
	}

	/**
	 * 批量修改默认hj_logistics_post_list
	 * 
	 * 项目名称：Shopping 类描述： 创建人：PC1 创建时间：2020年3月21日 上午10:52:57
	 * 
	 * @version
	 */
	...
	public String ij(@RequestBody Map<String, String> map) {
		...
		Gson gson = new Gson();
		return gson.toJson(
				hjServices.selalls2(String.valueOf(map.get("ij")), userService.Justiactions(username()).getStandby1()));
	}
	...
	public List<Object> getPosts(@RequestBody List<Map<String, Object>> map) {
		Gson gson = new Gson();
		/**
		 * 1.获取到需要发送的列表 2.进行不合格筛选 3.发送 4.获取到结果集 5.结果存入结果集返回
		 */
		// 存储结果
		...
		map.forEach(x -> {
			...
			resulte[0] = "";
			lists.forEach(y -> {
				if (x.get("shippingMethod").toString().equals(y.getShortName())
						|| x.get("shippingMethod").toString().equals(y.getStandy5())) {
			...
			JSONObject jsonObject2 = new JSONObject(
					gson.toJson(x.get("itemArr")).substring(1, gson.toJson(x.get("itemArr")).length() - 1));
			String namese = logout.namese(listsew, jsonObject.getString("consigneeName"),
					jsonObject2.getString("invoiceCnname"), jsonObject2.getString("invoiceEnname"));
			if (namese != "0") {
				maps.put("code", "0");
				maps.put("data", "");
			...
			String apiurl = urls + "api/logistics/v1/track/sync/create/order";// http 异步 请求路径
			// http请求请求
			try {
				// 3
				...
				String doPost = HttpUtilss.doPost(apiurl, gson.toJson(map2), gson.toJson(map2), EnumContentType.JSON,
						30000);
		 ...
		 * logger.info("价格大于11，不上传"); return "价格大于11，不上传"; }; //查询所有的物流及限制
		 * List<logOutOf> lists=logout.selectAll(); final String[] result = new
		 * String[1]; result[0]=""; lists.forEach(x->{
		 * if(map.get("shippingMethod").toString().equals(x.getShortName())||map.get(
		 * "shippingMethod").toString().equals(x.getStandy5())){
		 * if(Double.parseDouble(map.get("orderWeight").toString())>=Double.parseDouble(
		 * x.getStandy4())){ result[0]="重量超过"+x.getStandy4()+"kg"; return; }; } });
		 * if(!result[0].equals("")){ return gson.toJson(result[0]); } JSONObject
		 * jsonObject = new JSONObject(gson.toJson(map.get("consignee"))); JSONObject
		 * jsonObject2 = new JSONObject(gson.toJson(map.get("itemArr")).substring(1,
		 * gson.toJson(map.get("itemArr")).length()-1));
		 *//**
			 * 拦截收件人名称
			 *//*
				 * 
				 * String
				 * namese=logout.namese(jsonObject.getString("consigneeName"),jsonObject2.
				 * getString("invoiceCnname"),jsonObject2.getString("invoiceEnname"));
				 * if(namese!="0"){ return gson.toJson(namese); }
				 * 
				 * String apiurl=urls+"api/logistics/v1/track/sync/create/order";//http 请求路径
				 * 
				 * //http请求请求 try { map.put("shipperHawbcode",
				 * map.get("referenceNo").toString()); logger.info(gson.toJson(map)); String
				 * doPost = HttpUtilss.doPost(apiurl, gson.toJson(map),
				 * gson.toJson(map),EnumContentType.JSON, 30000); logger.info(doPost); return
				 * doPost; } catch (Exception e) { // TODO Auto-generated catch block
				 * e.printStackTrace(); logger.error("发送错误",e); }
				 */
		// return null;
	}
	...
	public List<Object> asyncPostYu(@RequestBody List<Map<String, Object>> map) {
		 ...
		 * if(Double.parseDouble(map.get("orderWeight").toString())>=Double.parseDouble(
		 * x.getStandy4())){ result[0]="重量超过"+x.getStandy4()+"kg"; return; }; } });
		 * if(!result[0].equals("")){ return gson.toJson(result[0]); } String
		 * apiurl=urls+"api/logistics/v1/track/create/order";//http 请求路径
		 * 
		 * //http请求请求 try { String doPost = HttpUtilss.doPost(apiurl, gson.toJson(map),
		 * gson.toJson(map),EnumContentType.JSON, 30000); return doPost; } catch
		 * (Exception e) { // TODO Auto-generated catch block logger.error("异步发送失败",e);
	...
	/**
	 * 义达批量删除 yddel 项目名称：Shopping 类描述： 创建人：PC1 创建时间：2020年3月16日 下午11:50:32
	 * 
	 * @version
	 */
	...
	@ResponseBody
	public String getpdfall(@RequestBody List<String> map, HttpServletRequest request)
		...
		for (hjBase bjbase : hi) {
			...
			String c = orderLable(maps);
			System.out.println(c);
			if (c == null) {
				continue;
			}
			JSONObject jsonObj = new JSONObject(c);
			Object jsonsI = jsonObj.get("body");
			if (jsonsI.equals(null) || jsonsI == "") {
				continue;
			}
			JSONObject result = (JSONObject) jsonsI;
			// 循环添加要合并的pdf存放的路径，path是进行路径生成
			mergePdf.addSource(path(result.get("data").toString(), request, bjbase.getHjShippingmethod()));
	...
	// 标签下载2.0/Logistics/getpdfall
	public String orderLable(@RequestBody Map<String, Object> map) {
		try {
			return HJ_API_CONNECT.doGet(HJApiEnum.LABLE.getUri() + map.get("lableKey"), null, EnumContentType.JSON,
					30000);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			return null;
	...
	public String conserveFile(@RequestBody Map<String, Object> map, HttpServletRequest request) {
		...
		try {
			...
			/*
			 * File file = new File(filePath); if (file.exists()) { //如果文件存在，则删除文件
			 * file.delete(); }
			 */
			Files.copy(inputStreams, Paths.get(filePath));
			Gson gson = new Gson();
			// 添加水印
			TestwaterMark.waterMark(filePath, path + uuid + map.get("fileName") + "1.pdf", "P001",
					map.get("metho").toString());
	...
	public int setLableid(@RequestBody List<Map<String, Object>> list) {
		...
		/*
		 * return
		 * logisticsService.insertLableKey(map.get("id").toString(),map.get("lab").
		 * toString(),map.get("num1").toString(),map.get("id2").toString(),map.get(
		 * "methosd").toString()
	...
	public String dropinfo(@RequestBody Map<String, Object> map, HttpServletRequest req) {
		...
		try {
			...
			// 传入唯一编号
			List<hjBase> y = hjServices.getNoList(map.get("referenceNoList").toString(),
					userService.Justiactions(username()).getStandby1());
			if (y.size() == 0) {
			...
			String doPost2 = HttpUtilss.doPost(apiurl2, gson.toJson(resMap), gson.toJson(resMap), EnumContentType.JSON,
					30000);
			// String
			// s="{\"status\":\"1\",\"errormsg\":null,\"errorcode\":null,\"spenttime\":162,\"result\":{\"succList\":[\"4_1189145774647377922\",\"4_1189157442131558402\",\"4_1189158008123523073\",\"4_1189160009481814017\",\"4_1189160318211948546\",\"4_1189161485075709953\"]},\"errorlevel\":null}";
			 ...
			 * 修改提交的编号
			 */
			// String[] p=map.get("referenceNoList").toString().substring(1,
			// map.get("referenceNoList").toString().length()-1).split(",");
	 ...
	 * 获取国家二字码
	 * 
	 * 项目名称：Shopping 类描述： 创建人：PC1 创建时间：2020年3月21日 上午11:18:42
	 * 
	 * @version
	 */
	...
	public int ALLbatch(@RequestBody Map<String, String> map) {
		...
		List<String> hjList2 = new ArrayList<String>();
	...
	public String yidaLogistics(@RequestBody Map<String, Object> map, String string) throws Exception {
		...
		Gson gson = new Gson();
		String doPost = HttpUtilss.doydPost(apiurl2, gson.toJson(map), EnumContentType.URLEncoder, 3000000, string);
		return doPost;
	}

	/**
	 * 发送创志订单 yidaLogistics
	 * 
	 * @param string
	 * @throws Exception
	 */
	...
	public int addyida(@RequestBody ydBase base) throws Exception {
		...
		// 其他的默认不选中
		Map<String, Object> data = new HashMap<String, Object>();
		data.put("fids", Arrays.asList(base.getYdStandy3()));
		defaults.BatchDefaultAllOrders(data);
	...
	public int addyidas(@RequestBody Map<String, Object> map) throws Exception {
		...
		List<String> hjbasee = new ArrayList<String>();
		...
		logisticsService.selectListid(map.get("checkedCities").toString()).forEach(x -> {
			...
			base.setYdReferenceNo("YD" + SnowIdUtils.uniqueLong());
			...
			base.setYdStandy2("1");
			base.setYdStandy8(username);
			hjbasee.add(x.getfIds());
	...
	@ResponseBody
	public int ydDelOrder(@RequestBody ydBase base) throws Exception {

//		Map<String,Object> map=new HashMap<String,Object>();
//		map.put("reference_no", base.getYdReferenceNo());
//		return yidaLogistics(map,"removeorder");
		return yida.ydDelOrder(base);
	}
	...
	public List<Object> postYds(@RequestBody List<String> list) throws Exception {
		...
		if (list.size() > 0) {
			...
			integerList2 = integerList1.stream().sorted().filter(x -> !"".equals(x)).distinct()
					.collect(Collectors.toList());
		}

		yida.selectYds(logisticsService.selectIDList(integerList2)).forEach(base2 -> {
			...
			if ("".equals(base2.getYdConsigneeProvince()) || base2.getYdConsigneeProvince() == null) {
				consignee.put("consignee_province", base2.getYdConsigneeCity());
			} else {
			...
			consignee.put("consignee_email", base2.getYdConsigneeEmail());
			// 海关申报信息
	...
	public Map<String, Object> orderlist(@RequestBody Map<String, Object> maps) throws Exception {
		 ...
		 */
		String username = userService.Justiactions(username()).getStandby1();
		map.put("hj", hjServices.selectHjLogice(maps.get("fid").toString(), username, maps.get("id").toString()));
		map.put("yd", yida.selectOrder(maps.get("fid").toString(), username, maps.get("id").toString()));
		map.put("cz", czservice.selectOrder(maps.get("fid").toString(), username));
		map.put("sf", sfServices.selectOrder(maps.get("fid").toString(), username));
		map.put("eub", eubServices.selectOrder(maps.get("fid").toString(), username));
		map.put("hm", hmService.detectOrder(maps.get("fid").toString()));
	...
	public String ydPrint(@RequestBody List<String> list) throws Exception {
		...
		yida.selectIDList(logisticsService.selectIDList(integerList2)).forEach(sx -> {
	...
	public String czPrint(@RequestBody List<String> list, HttpServletRequest req) throws Exception {
		...
		map1.put("access_token", "5e82a8df3aff972da61d29225e82a8df2a7736220");
		czservice.selectIDList(logisticsService.selectIDList(integerList2)).forEach(x -> {
		...
		lists.forEach(x -> {
			...
			Map<String, Object> data1 = gson.fromJson(x.toString(), new TypeToken<Map<String, Object>>() {
			}.getType());
			if (!data1.get("data").toString().equals("[]")) {
				Map<String, Object> data2 = gson.fromJson(gson.toJson(data1.get("data")),
						new TypeToken<Map<String, Object>>() {
						}.getType());
				Map<String, Object> data3 = gson.fromJson(gson.toJson(data2.get("shipment")),
						new TypeToken<Map<String, Object>>() {
						}.getType());
				/**
				 * 添加拼接地址
	...
	public int addOrder(@RequestBody Map<String, Object> map) throws Exception {
		...
		if (map.get("type").equals("1") || map.get("type").equals("3")) {
			// 添加
			// List<String> ids=gson.fromJson(map.get("ids").toString(),new
			...
			List<String> hjbasee = new ArrayList<String>();
			logisticsService
					.selectListid(map.get("ids").toString().substring(1, map.get("ids").toString().length() - 1))
					.forEach(x -> {
	...
	public List<Object> czpost(@RequestBody List<String> list) throws Exception {
		...
		// 创志默认token
		map1.put("access_token", "5e82a8df3aff972da61d29225e82a8df2a7736220");
		czservice.selectIDList(logisticsService.selectIDList(integerList2)).forEach(base2 -> {
			/**
			 * 第一级
			 */
			Map<String, Object> map = new HashMap<String, Object>();
			 ...
			 */
			/*
			 * Map<String,Object>map3=new HashMap<String,Object>(); map3.put("name",
			 * base2.getToName()); map3.put("tel", base2.getToTel()); map3.put("address_1",
			 * base2.getToAddress1()); map3.put("city", base2.getToCity());
			 * map3.put("country", base2.getToCountry()); map3.put("postcode",
			 * base2.getToPostcode());
			 */
			/**
			 * 收件人to_address
	 ...
	 * 添加创志跟踪号
	 * 
	 * 项目名称：Shopping 类描述： 创建人：PC1 创建时间：2020年4月1日 下午7:20:29
	 * 
	 * @version
	 */
	...
	public String btdadd() throws Exception {
		...
		Gson gson = new Gson();
		try {
			doposte = HttpUtilss.doPost2("http://open.btdair.com:8099/LogisticsService.svc?wsdl", sb.toString(),
					EnumContentType.XML);
